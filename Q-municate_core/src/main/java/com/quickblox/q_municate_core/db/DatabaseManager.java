...
//                USER_FRIEND_RELATION_KEY + " AND (" + condition + ")", null, sortOrder);
        Cursor cursor = resolver.query(UserTable.USER_FRIEND_CONTENT_URI, null,
                USER_FRIEND_RELATION_KEY, null, sortOrder);
...
//                USER_FRIEND_RELATION_KEY + " AND (" + condition + ")", null, null);
        Cursor cursor = resolver.query(UserTable.USER_FRIEND_CONTENT_URI, new String[]{"count(*)"},
                USER_FRIEND_RELATION_KEY, null, null);
...
//                " AND " + FriendTable.TABLE_NAME + "." + FriendTable.Cols.IS_STATUS_ASK + " = 1" + ")) AND " + UserTable.TABLE_NAME + "." + UserTable.Cols.USER_ID + " NOT IN " + condition;
        String conditionResult = UserTable.TABLE_NAME + "." + UserTable.Cols.USER_ID + " NOT IN " + condition;
    ...
    public static CursorLoader getAllGroupDialogsCursorLoader(Context context) {
        return new CursorLoader(context, DialogTable.CONTENT_URI, null,
                DialogTable.Cols.TYPE + "= ? OR " + DialogTable.Cols.TYPE + "= ?",
                new String[]{String.valueOf(QBDialogType.PUBLIC_GROUP.getCode()), String.valueOf(QBDialogType.GROUP.getCode())},
                DialogTable.Cols.ID + " ORDER BY " + DialogTable.Cols.LAST_DATE_SENT + " DESC");
    }
    ...
    public static QBDialog getDialogFromCursor(Cursor cursor) {
        ...
        Integer userId = cursor.getInt(cursor.getColumnIndex(DialogTable.Cols.USER_ID));
    ...
    public static User getUserFromCursor(Cursor cursor) {
        ...
        String externalid = cursor.getString(cursor.getColumnIndex(UserTable.Cols.EXTERNAL_ID));
        boolean online = cursor.getInt(cursor.getColumnIndex(UserTable.Cols.IS_ONLINE)) > 0;

        User user = new User(id, fullName, email, phone, avatarUid, externalid);
    ...
    public static void saveChatMessages(Context context, List<QBChatHistoryMessage> messagesList,
        ...
        for (QBChatHistoryMessage historyMessage : messagesList) {
            ...
            // bronislaw temp
            if (message.equals("join") || message.equals("stay") || message.equals("leave"))
                continue;
    ...
    private static ContentValues getContentValuesForCreateDialogTable(QBDialog dialog) {
        ...
        values.put(DialogTable.Cols.USER_ID, dialog.getUserId());
        values.put(DialogTable.Cols.NAME, dialog.getName());
        values.put(DialogTable.Cols.COUNT_UNREAD_MESSAGES, dialog.getUnreadMessageCount());
