...
public class ChatUtils {
    ...
    public static ArrayList<User> joinUserLists = new ArrayList<User>();
    public static String joinDialogId = null;
    ...
    public static void setJoinUsers(User sender) {
        User tempUser;
        boolean isExist = false;

         if (joinUserLists.size() == 0) {
             joinUserLists.add(sender);
         } else {
             for (int i = 0; i < joinUserLists.size(); i++) {
                 tempUser = joinUserLists.get(i);
                 if (tempUser == sender) {
                     isExist = true;
                     break;
                 }
    ...
    public static void removeJoinUsers(User delSender) {
        User tempUser;

        for (int i = 0; i < joinUserLists.size(); i++) {
            if (joinUserLists.get(i).getUserId() == delSender.getUserId())
                joinUserLists.remove(delSender);
        }
    }

    public static ArrayList<User> getJoinUsers() {
        return joinUserLists;
    }

    /**
     * Paypal payment
     */
    public static int credits_count = 0;

    public static void setCredits_count(int count) {
        credits_count = count;
    }

    public static int getCredits_count() {
