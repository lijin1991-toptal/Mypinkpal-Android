...
public class QBPrivateChatHelper extends BaseChatHelper implements QBPrivateChatManagerListener {
    ...
    public void sendOccupantStatusPrivate(String sendStr, int userId) throws QBResponseException {
        QBChatMessage chatMessage;
        chatMessage = getQBChatMessage(sendStr, null);
        ...
        if (privateChat == null) {
            try {
                privateChat = createChatIfNotExist(userId);
            } catch (QBResponseException qe) {
                ErrorUtils.logError(qe);
            }
        }
        String error = null;
        try {
                ...
                if (chatMessage.getBody().equals("join") || chatMessage.getBody().equals("stay")) {
                    if (dialogId.equals(ChatUtils.joinDialogId) && user != null) {
                        ...
                        if (chatMessage.getBody().equals("join")) {
                            try {
                                sendOccupantStatusPrivate("stay", user.getUserId());
