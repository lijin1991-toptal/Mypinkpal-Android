...
public class VideoRightFragment extends ListFragment {
	...
	public void onListItemClick(ListView listview, View view, int position,
			long id) {
		Fragment newContent = new VideoFragment();

		Fragment newContentRight = new VideoRightFragment();
	...
	private class SideBarTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			try {
				...
				pairs.add(new BasicNameValuePair("method", "accountapi.getVideoCategories"));
		...
		protected void onPostExecute(String result) {
			try {
				...
				if (categoryId.equals("0")) {
					sa.addHeader(phraseManager.getPhrase(getActivity().getApplicationContext(), "video.categories"));
				} else {
