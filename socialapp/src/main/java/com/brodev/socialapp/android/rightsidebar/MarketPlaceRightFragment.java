...
import com.brodev.socialapp.fragment.MarketPlaceFragment;
 ...
 * @author huy nguyen
 */
public class MarketPlaceRightFragment extends ListFragment {

	// init Network Until
	private NetworkUntil networkUntil = new NetworkUntil();
	private SidebarAdapter sa = null;
	String categoryId = "0", categoryName;
	private User user;
	...
	public static MarketPlaceRightFragment newInstance() {
		return new MarketPlaceRightFragment();
	}
	...
	public void onListItemClick(ListView listview, View view, int position,
			long id) {
		Fragment newContent = new MarketPlaceFragment();

		Fragment newContentRight = new MarketPlaceRightFragment();
	...
	private class SideBarTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			try {
				...
				pairs.add(new BasicNameValuePair("method", "accountapi.getMarketPlaceCategories"));				
		...
		protected void onPostExecute(String result) {
			try {
				...
				if (categoryId.equals("0")) {
					sa.addHeader(phraseManager.getPhrase(getActivity().getApplicationContext(), "marketplace.categories"));
				} else {
