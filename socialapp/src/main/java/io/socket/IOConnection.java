...
class IOConnection implements IOCallback {
	 ...
	 */
	static public IOConnection register(String origin, SocketIO socket, List<NameValuePair> parList) {
		...
		IOConnection connection = new IOConnection(origin, socket, parList);
		list.add(connection);
		return connection;
	}
	...
	private void handshake() {
		...
		try {
			...
			if (lstParams != null) {
				url = new URL(IOConnection.this.url.toString() + SOCKET_IO_1 + lstParams);
			} else {
	...
	private String lstParams = null;
	
	private IOConnection(String url, SocketIO socket, List<NameValuePair> params) {
		try {
			this._params = params;
			if (_params != null) {
				lstParams  += "?" + URLEncodedUtils.format(params, "utf-8");
			}
	...
	public void transportMessage(String text) {
		...
		case IOMessage.TYPE_EVENT:
			try {
				...
				if ("online".equals(event.getString("name")) || "offline".equals(event.getString("name"))) {
					break;
				}
				Object[] argsArray;
				if (event.has("args")) {
