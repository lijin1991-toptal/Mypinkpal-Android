...
public class SocketIO {
	 ...
	 */
	public SocketIO(final String url, List<NameValuePair> pairs) throws MalformedURLException {
		if (url == null)
			throw new RuntimeException("url may not be null.");
		setAndConnect(new URL(url), null, pairs);
	}
	 ...
	 *             the malformed url exception
	 */
	public SocketIO(final String url, Properties headers, List<NameValuePair> pairs)
			throws MalformedURLException {
		if (url == null)
			throw new RuntimeException("url may not be null.");
	 ...
	 *             the malformed url exception
	 */
	public SocketIO(final String url, final IOCallback callback, List<NameValuePair> pairs)
			throws MalformedURLException {
		connect(url, callback, pairs);
	}
	 ...
	 *            the callback
	 */
	public SocketIO(final URL url, final IOCallback callback, List<NameValuePair> pairs) {
		if (setAndConnect(url, callback, pairs) == false)
	 ...
	 *            the url
	 */
	public SocketIO(final URL url, List<NameValuePair> pairs) {
		setAndConnect(url, null, pairs);
	}
	 ...
	 *            the callback
	 */
	public void connect(final String url, final IOCallback callback, List<NameValuePair> pairs)
			throws MalformedURLException {
		if (setAndConnect(new URL(url), callback, pairs) == false) {
	 ...
	 *            the callback
	 */
	public void connect(URL url, IOCallback callback, List<NameValuePair> pairs) {
		if (setAndConnect(url, callback, pairs) == false) {
			if (url == null || callback == null)
				throw new RuntimeException("url and callback may not be null.");
			else
	 ...
	 *            the callback
	 */
	public void connect(IOCallback callback, List<NameValuePair> pairs) {
		if (setAndConnect(null, callback, pairs) == false) {
			if (callback == null)
	 ...
	 */
	private boolean setAndConnect(URL url, IOCallback callback, List<NameValuePair> pairs) {
		...
		if (this.callback != null && this.url != null) {
			...
			this.connection = IOConnection.register(origin, this, pairs);
			return true;
		}
		return false;
	}
